...
export function InsightsList({ data }: InsightsListProps) {
  ...
  return (
    <motion.div
      ...
      className="rounded-3xl bg-linear-to-br from-amber-50 via-white to-emerald-50 border border-amber-100/50 p-8"
    >
      <div className="flex items-center gap-3 mb-6">
        <div className="w-12 h-12 rounded-xl bg-linear-to-br from-amber-500 to-amber-600 flex items-center justify-center text-white shadow-lg">
      ...
      <div className="space-y-4">
        {actionable_insights.map((insight, index) => (
          ...
          >
            ...
            <CheckCircle2 className="shrink-0 w-5 h-5 text-gray-300 hover:text-emerald-600 transition-colors cursor-pointer" />
          </motion.div>
        ))}
      </div>
    </motion.div>
